  ...
  return {
    ...
    async retrieveBlocks(blockId, depth = 0) {
      ...
      for (const block of blocks) {
        const syncedFromBlockId = block.type === "synced_block"
          ? block.synced_block?.synced_from?.block_id
          : "";
        if (block.has_children || syncedFromBlockId) {
          const childrenBlockId = syncedFromBlockId || block.id;
          const children = await this.retrieveBlocks(childrenBlockId, depth + 1).catch(() => []);
...
function blocksToArticleContent(blocks) {
  ...
  blocks.forEach((block) => {
    if (block.type === "heading_1" || block.type === "heading_2" || block.type === "heading_3") {
      ...
      appendBlockChildren(content, block);
      return;
    }
    ...
    if (["column_list", "column", "table", "synced_block", "template"].includes(block.type)) {
      appendBlockChildren(content, block);
      return;
    }

    if (["table_of_contents", "breadcrumb", "child_page", "child_database"].includes(block.type)) {
...
function appendBlockChildren(content, block) {
  if (!Array.isArray(block.children) || !block.children.length) return;
  content.push(...blocksToArticleContent(block.children).filter((child) => !isEmptyArticlePlaceholder(child)));
}

function isEmptyArticlePlaceholder(block) {
  return block?.type === "paragraph" && block.text === "这篇文章还没有正文。";
}
