...
function blocksToArticleContent(blocks) {
  ...
  blocks.forEach((block) => {
    if (block.type === "heading_1" || block.type === "heading_2" || block.type === "heading_3") {
      content.push({ type: "heading", text: getBlockPlainText(block[block.type]), level: Number(block.type.slice(-1)) });
      return;
    }
    ...
    if (block.type === "bulleted_list_item" || block.type === "numbered_list_item") {
      ...
      if (last?.type === "list" && last.ordered === ordered) {
        ...
        content.push({ type: "list", ordered, items: [text] });
      }
      return;
    }
    ...
    if (block.type === "to_do") {
      ...
      if (text) content.push({ type: "todo", text, checked: Boolean(block.to_do.checked) });
      return;
    }

    if (block.type === "callout") {
      const text = getBlockPlainText(block.callout);
      if (text) content.push({ type: "callout", text, icon: getNotionIconText(block.callout.icon) });
      return;
    }
    ...
    if (["toggle", "table_of_contents", "breadcrumb", "child_page", "child_database", "synced_block", "template"].includes(block.type)) {
